...
**프로세서 보드를 올린 면이 pcb의 앞면이 되고, 반대편 면은 뒷면이 됩니다. 왼손과 오른손에 맞춰 pcb를 뒤집어 프로세서 보드를 올려주세요.**

![processorSolder4](../images/dasicBuildProcessorSolder4.jpg)

pcb의 뒷면을 보고 헤더 핀과 점퍼를 모두 땜합니다.
...
각 손의 앞면 부분에 커넥터가 오도록 TRRS 커넥터를 배치하고, 뒷면에서 납땜합니다. 납땜할 때 마스킹 테이프 등으로 고정해두면 좋습니다.

![jackSolder3](../images/dasicBuildJackSolder3.jpg)

이 면을 보고 있는 상태에서, "JUMP when BACK SIDE"라는 문구 주변의 점퍼 세 점을 땜납으로 점프합니다. 반대쪽 면(=커넥터가 올라가 있는 윗면)은 하지 않습니다.

해당 점퍼들을 납땜할 땐 컨트롤러 보드의 뒷면 점퍼를 땜할 때처럼 하면 됩니다.
...
smd 스위치는 reset 핀과 연결되어있습니다. rp2040이 아닌 프로세서 보드는 이 스위치로 dfu 모드에 들어가게 됩니다.
...
마지막으로 백플레이트의 범폰 위치에 범폰 4조각을 붙이면 완성입니다.